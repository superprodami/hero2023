...
//fp32 test[3];
//fp32 test1[3];
kalman_filter_init_t IMU_kalman_filter_para =
{
    .xhat_data = {0, 0},
    .P_data = {1, 0, 0, 1},
    .A_data = {1, 0.001, 0, 1},//采样时间间隔
    .B_data = {0.001 * 0.001 * 0.5, 0.001},
    .Q_data = {1, 0, 0, 1},
    .H_data = {1, 0, 0, 1},
    .R_data = {0.5, 0, 0, 1}
};
kalman_filter_t IMU_kalman_filter;
float *IMU_kf_result;
...
void IMU_InitArgument(void)
{
    ...
    imu_temp_pid.f_param_init(&imu_temp_pid, PID_POSITION, imu_temp_PID, TEMPERATURE_PID_MAX_OUT, TEMPERATURE_PID_MAX_IOUT, 0, 0, 0, 360, 0);

    kalman_filter_init(&IMU_kalman_filter, &IMU_kalman_filter_para);
    AHRS_init(INS_quat, bmi088_real_data.accel, ist8310_real_data.mag);

    accel_fliter_1[0] = accel_fliter_2[0] = accel_fliter_3[0] = bmi088_real_data.accel[0];
    accel_fliter_1[1] = accel_fliter_2[1] = accel_fliter_3[1] = bmi088_real_data.accel[1];
    accel_fliter_1[2] = accel_fliter_2[2] = accel_fliter_3[2] = bmi088_real_data.accel[2];
...
uint32_t IMU_frequent =0 , IMU_cont = 0 ;


uint32_t count = 0;
fp32 insstart=0,insend=0,inslinpiao=0;
fp32 inslast;
fp32 testimuyaw,testimupitch;
void ImuFun(void const *pvParameters)
{
//    uint16_t count = 0;
//		fp32 imustart,imuend,imulinpiao;
    ...
    while(1)
    {
        ...
        //等待SPI DMA传输
			testimuyaw =IMU_angle[0];
			testimupitch=IMU_angle[1];
        ...
        if(!imu_init_finish_flag && count > 500)
        {
            ...
            count = 600;
				...
				{
					insend=INS_angle[0];
					inslinpiao=(insend-insstart)/2000.f;
				}
        ...
        IMU_angle[0] = 180.0f*INS_angle[0]/pi; //yaw               
				IMU_angle[1] = (180.0f*INS_angle[1]/pi)-15; //pitch
        IMU_angle[2] = 180.0f*INS_angle[2]/pi; //roll
				if(imu_init_finish_flag)
				vision_send();

//        set_angle(&hcan2,IMU_angle[0],imu_init_finish_flag);
...
static void imu_temp_control(fp32 temp)
{
    ...
    if (first_temperate)
    {
        imu_temp_pid.f_cal_pid(&imu_temp_pid, temp, 45.0f);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    else if(GPIO_Pin == SIGNAL_Pin)
    {
        gimbal_return_finish_flag = true;
    }
}
