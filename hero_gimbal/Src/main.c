...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  delay_init();
  MX_USB_DEVICE_Init();  //启动虚拟串口要放在freertos执行前 不然会启动失败
...
//    __HAL_UART_ENABLE_IT(&huart6, UART_IT_IDLE);  //本来用于和视觉通信，现在采用虚拟串口通信
//    HAL_UART_Receive_DMA(&huart6, ALL_DATA_BUFF, 12);
